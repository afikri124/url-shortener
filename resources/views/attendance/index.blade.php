        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="text-center pt-4">
                    ...
                    @if($data->expired != null && $data->expired > \Carbon\Carbon::now())
                    <div id="countdown">
                        <ul style="padding: 0;">
            ...
            @if($data->expired != null && $data->expired < \Carbon\Carbon::now() && $check==null ) 
            <div
                    class="alert alert-danger text-center" role="alert">
                    Maaf, Anda sudah tidak bisa melakukan Absensi<br>karena absensi
                    {{ $data->title }} {{ $data->sub_title }}
                    <br>telah berakhir pada
                    {{ \Carbon\Carbon::parse($data->expired)->translatedFormat("l, d F Y H:i");}}
            </div>
            @else
            <form id="formAuthentication" class="mb-3 row" action="" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="col-md-6">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('jabatan') is-invalid @enderror" id="jabatan"
                            name="jabatan" value="{{ (old('jabatan') == null ? Auth::user()->job : old('jabatan')) }}"
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="judul" value="{{ $data->title }}" disabled />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Lokasi/Tempat @if($data->type == "M") Rapat @else Acara
                            @endif</label>
                        <input type="text" class="form-control" name="lokasi" value="{{ $data->location }}" disabled />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Hari/Tanggal</label>
                        <input type="text" class="form-control" name="tanggal"
                            value='{{ \Carbon\Carbon::parse($data->date)->translatedFormat("l, d F Y"); }}' disabled />
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label">Paraf Digital <strong class="text-danger">*</strong></label><br>
                        @if($check !=null)
                        <img src="{!! $check->signature_img !!}" style="width: 300px; height:150px;" alt="Paraf" />
                        @else
                ...
                <div class="mb-3 text-center">
                    <button class="btn btn-success w-100" type="submit" name="attendance" @if($check !=null) disabled
                        @endif><i class="bx bx-check-circle me-2"></i>Absen Sekarang</button>
                </div>
                <div class="mb-3 text-center">
...
<script type="text/javascript">
    ...
    function show_position(p) {
        ...
        var lonLat = new OpenLayers.LonLat("{{ ($check == ''? '': $check->longitude) }}", "{{ ($check == ''? '': $check->latitude) }}")
            .transform(
                new OpenLayers.Projection("EPSG:4326"), // transform from WGS 1984
                map.getProjectionObject() // to Spherical Mercator Projection
            );
